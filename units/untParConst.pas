 ...
 *****************************************************************************
 ...
 Unit description:
 Constants and related Get-functions used by other units
}
...
type
  ...
  TTypeMatrix = (DX7, DX7II, TX7, MDX, PSSx80);
...
const
  ...
  PSSx80_NAMES: array [0..36, 0..1] of string = (
    ('BANK', 'BANK'),
    ('M: Detune Fine', 'M_DT1'),
    ('M: Freq. Multiplicator', 'M_MUL'),
    ('M: Modulation Level', 'M_TL'),
    ('M: Level Key Scaling Hi', 'M_LKS_HI'),
    ('M: Level Key Scaling Lo', 'M_LKS_LO'),
    ('M: Rate Key Scaling', 'M_RKS'),
    ('M: Attack Rate', 'M_AR'),
    ('M: AM Enable', 'M_AM_EN'),
    ('M: Detune Coarse', 'M_DT2'),
    ('M: Decay 1 Rate', 'M_D1R'),
    ('M: Sine Table Form', 'M_SIN_TBL'),
    ('M: Decay 2 Rate', 'M_D2R'),
    ('M: Decay 1 Level', 'M_D1L'),
    ('M: Release Rate', 'M_RR'),
    ('M: Sustain Release Rate', 'M_SRR'),
    ('M: Feedback', 'M_FB'),
    ('C: Detune Fine', 'C_DT1'),
    ('C: Freq. Multiplicator', 'C_MUL'),
    ('C: Total Level', 'C_TL'),
    ('C: Level Key Scaling Hi', 'C_LKS_HI'),
    ('C: Level Key Scaling Lo', 'C_LKS_LO'),
    ('C: Rate Key Scaling', 'C_RKS'),
    ('C: Attack Rate', 'C_AR'),
    ('C: AM Enable', 'C_AM_EN'),
    ('C: Detune Coarse', 'C_DT2'),
    ('C: Decay 1 Rate', 'C_D1R'),
    ('C: Sine Table Form', 'C_SIN_TBL'),
    ('C: Decay 2 Rate', 'C_D2R'),
    ('C: Decay 1 Level', 'C_D1L'),
    ('C: Release Rate', 'C_RR'),
    ('C: Sustain Release Rate', 'C_SRR'),
    ('Pitch Modulation Sensitivity', 'PMS'),
    ('Amplitude Modulation Sensitivity', 'AMS'),
    ('Vibrato Delay Time', 'VDT'),
    ('Vibrato On', 'V'),
    ('Sustain On', 'S')
    );

  PSSx80_VCED_MIN_MAX_INT: array [0..71, 0..2] of byte = (
    (0, 5, 0),
    (0, 15, 0),
    (0, 15, 1),
    (0, 99, 99),
  ...
  //PSS
  transSIN_TBL: array[0..3] of string =
  ...
  try
    case T of
      ...
      PSSx80: case V of
          fmin: begin
            for i := low(Ret) to high(Ret) do
              Ret[i] := PSSx80_VCED_MIN_MAX_INT[i][0];
          end;
          fmax: begin
            for i := low(Ret) to high(Ret) do
              Ret[i] := PSSx80_VCED_MIN_MAX_INT[i][1];
          end;
          finit: begin
            for i := low(Ret) to high(Ret) do
              Ret[i] := PSSx80_VCED_MIN_MAX_INT[i][2];
          end;
        end;
    end;
  except
